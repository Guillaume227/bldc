...
namespace packet{
  ...
  struct PACKET_STATE_t{
	...
	unsigned char rx_buffer[MAX_PL_LEN];
	unsigned char tx_buffer[MAX_PL_LEN + 6];
	unsigned int rx_data_ptr;
	unsigned char crc_low;
	unsigned char crc_high;
  };

  PACKET_STATE_t handler_states[NUM_HANDLERS];
  ...
  void process_byte(uint8_t rx_data, int handler_num) {
	...
	case 2:
		...
		if (handler_states[handler_num].payload_length > 0 &&
				handler_states[handler_num].payload_length <= MAX_PL_LEN) {
			handler_states[handler_num].rx_state++;
			handler_states[handler_num].rx_timeout = RX_TIMEOUT;
		} else {
			handler_states[handler_num].rx_state = 0;
