...
namespace app{
  namespace nunchuk{
	...
	// Threads
	THD_FUNCTION(chuk_thread, arg);
	THD_WORKING_AREA(chuk_thread_wa, 1024);
	THD_FUNCTION(output_thread, arg);
	THD_WORKING_AREA(output_thread_wa, 1024);

	// Private variables
	volatile bool stop_now = true;
	volatile bool is_running = false;
	chuck_data chuck_d;
	volatile int chuck_error = 0;
	chuk_config config;
	volatile bool output_running = false;
	volatile systime_t last_update_time;

	// Private functions
	void terminal_cmd_nunchuk_status(int argc, const char **argv);

	void configure(chuk_config *conf) {
		config = *conf;

		terminal::register_command_callback(
				"nunchuk_status",
	...
	THD_FUNCTION(output_thread, arg) {
		...
		for(;;) {
			...
			if (timeout::has_timeout() || chuck_error != 0 || config.ctrl_type == CHUK_CTRL_TYPE_NONE) {
				continue;
			}
	...
	void terminal_cmd_nunchuk_status(int argc, const char **argv) {
		...
		printf("Nunchuk Status");
		printf("Output: %s", output_running ? "On" : "Off");
		printf(" ");
	}
  }
}
