...
namespace encoder{
  ...
  //	palSetPadMode(HW_HALL_ENC_GPIO3, HW_HALL_ENC_PIN3, PAL_MODE_ALTERNATE(HW_ENC_TIM_AF));
      ...
      TIM_EncoderInterfaceConfig (HW_ENC_TIM, TIM_EncoderMode_TI12,
              ...
              TIM_ICPolarity_Rising);
      TIM_SetAutoreload(HW_ENC_TIM, m_enc_counts - 1);
  ...
  void init_as5047p_spi(void) {
      ...
      m_mode = ENCODER_MODE_AS5047P_SPI;
      m_index_found = true;
  }
  ...
  void reset(void) {
      ...
      if (palReadPad(HW_HALL_ENC_GPIO3, HW_HALL_ENC_PIN3)) {
          ...
          const unsigned int lim = m_enc_counts / 20;

          if (m_index_found) {
              // Some plausibility filtering.
              if (cnt > (m_enc_counts - lim) || cnt < lim) {
                  HW_ENC_TIM->CNT = 0;
                  bad_pulses = 0;
              } else {
                  bad_pulses++;
  ...
  bool index_found(void) {
      return m_index_found;
  }

  // Software SPI
  void spi_transfer(uint16_t *in_buf, const uint16_t *out_buf, int length) {
